...
class Image extends AbstractFrameReflower
{
    ...
    function get_min_max_width()
    {
        ...
        $width = ($style->width > 0 && $style->width!='auto'? $style->width : 0);
        ...
        $height = ($style->height > 0 && $style->height!='auto' ? $style->height : 0);
        ...
        if ($style->min_width !== "none" ||
            ...
            if (Helpers::is_percent($max_width)) {
                $limitCount=0;
                $parent=$this->_frame->get_parent();

                while ($parent!=null&&$limitCount<1000) {
                    try {
                        $limitCount++;
                        if ($limitCount == 1000)
                            break;
                        ...
                        $dimensions=null;
                        if(method_exists($parent->get_reflower(),'_calculate_restricted_width'))
                        {
                            $restrictedWidth=$parent->get_reflower()->_calculate_restricted_width();
                            if(is_array($restrictedWidth))
                                $dimensions['w']=$restrictedWidth[0];
                        }
                        if($dimensions==null)
                            $dimensions=$parent->get_containing_block();
                        if($dimensions!=null&&isset($dimensions['w'])) {
                            $max_width = $dimensions['w'];
                            $parentStyles=$this->_frame->get_parent()->get_style();
                            if($parentStyles!=null&&$parentStyles->padding!=null)
                            {
                                $points=$parentStyles->length_in_pt($parentStyles->padding);
                                $max_width-=($points*2);
